...
def run_co_scientist_workflow(
    ...
    This function orchestrates a multi-round AI co-scientist workflow:
      ...
      6) Remove the weakest proposals in each round (lower half, third, quarter...)
    ...
    # For round 0 => remove half, round 1 => remove third, round 2 => remove quarter, etc.
    # If more than 3 rounds are used, we'll default to removing a quarter in subsequent.
    ...
    for round_idx in range(num_rounds):
        ...
        if round_idx == 0:
            ...
            # In subsequent rounds, we first remove the weakest proposals, then
            # generate new ones in their place, or evolve existing ones.
            fraction = removal_fractions[round_idx] if round_idx < len(removal_fractions) else 0.25

            # The ranking is done at the *end* of the last round iteration, so let's do it after we
            # gather the reflection/proximity steps, to keep the logical order:
            # Actually, let's shuffle the steps: we must do Reflection/Proximity/Ranking on existing
            # ideas from the previous round, then remove the weaker, then generate new ones, THEN
            # do Reflection/Proximity/Ranking again for the new set. But for simplicity and shortness,
            # we can do one reflection/proximity/ranking cycle, remove, then fill, then skip a second cycle
            # or do a short reflection. This detail can vary. 
            #
            # We'll do it in the same order as the first round, but the difference is after the ranking
            # we remove the weaker proposals and replace them with newly generated ones.
            ...
            # Update our ideas with the newly evolved set
            ideas = evolved_ideas

        # 2) Reflect on the ideas (check plausibility, novelty, correctness)
        ideas_text = "\n".join([f"{i+1}. {idea}" for i, idea in enumerate(ideas)])
        reflection_prompt = (
        ...
        if round_idx < len(removal_fractions):
            fraction = removal_fractions[round_idx]
        else:
            # If there are more than 3 rounds, remove the weaker quarter as default
            fraction = 0.25

        num_to_remove = int(len(ideas) * fraction)
        if num_to_remove < 1 and len(ideas) > 1:
            # Ensure at least 1 is removed if there's more than 1 idea total
            num_to_remove = 1

        if num_to_remove > 0:
            removed_ideas = ideas[-num_to_remove:]  # remove from the bottom (weakest)
    ...
    # After all rounds, we do a final ranking and then a Meta-review of only the top 5
    ideas_text = "\n".join([f"{i+1}. {idea}" for i, idea in enumerate(ideas)])
...
def parse_ideas_from_text(text: str, expected_count: int) -> List[str]:
    ...
    # No simple fallback if fewer than expected; you could pad or do something else
