...
DEBUG_MODE = True       # Enable detailed logging (can be controlled via --no-debug flag)
...
def run_co_scientist_workflow(
    ...
    2. Each round:
       ...
       - Evolution Agent can also generate new ideas (up to max_new_ideas_per_round)
    ...
    supervisor_intro = call_agent(
        ...
        user_prompt=(
            ...
            "rankings using eight criteria, including "
    ...
    for round_idx in range(num_rounds):
        ...
        if round_idx == 0:
            ...
            # Calculate how many new ideas we can add
    ...
    # Get the top N ideas for meta-review
    ...
    meta_prompt = (
        ...
        f"{final_ideas_text}\n\n"
        f"1. Summarize each of the top 5 ideas, highlighting their core hypotheses and key innovations\n"
...
def compute_idea_score_vectors(ideas: List[str], tournament_agent_prompt: str) -> Dict[str, IdeaScore]:
        ...
        for i in range(0, len(ideas), batch_size):
            ...
            batch_prompt = (
                "Score each of the following ideas across all eight criteria (1-10):\n\n" +
            ...
            for j, idea in enumerate(batch):
                ...
                if idea_match:
                    ...
                    scores = []
                    score_pattern = r'(?i)(?:criterion\s*\d+|plausibility|novelty|correctness|methodological|resource|impact|interdisciplinary|scalability)[^=]*=\s*(\d+)'
                    ...
                    # If we found exactly 8 scores, create the score vector
                    if len(scores) == 8:
                        score_vectors[idea] = IdeaScore(*scores)
                    else:
                        print(f"Warning: Found {len(scores)} scores for idea {j+1}, expected 8")
                        # Use default scores as fallback
                        score_vectors[idea] = IdeaScore(*([6.0] * 8))
...
def run_optimized_tournament(
    ...
    for i, idea_a in enumerate(ideas):
        ...
        for match_num, idea_b in enumerate(opponents, 1):
            ...
            print(textwrap.fill(idea_a[:200] + "...", width=80, initial_indent="  ", subsequent_indent="  "))
            print(f"\nIdea B (current ELO {elo_ratings[idea_b]:.1f}):")
            print(textwrap.fill(idea_b[:200] + "...", width=80, initial_indent="  ", subsequent_indent="  "))
            ...
            if idea_b not in match_results:
                match_results[idea_b] = []
            match_results[idea_b].append({
                'opponent': idea_a,
                'score': 1 - match_score,
                'vector_a': vec_b,
                'vector_b': vec_a
            ...
            for criterion, score_a, score_b in zip(criteria, list(vec_a), list(vec_b)):
        ...
        try:
            for idea in ideas:
                ...
                if idea_matches:
                    ...
                    # Include the idea's score vector
                    if idea in score_vectors:
                        criteria = [
                            "Plausibility", "Novelty", "Correctness", "Methodological Rigor",
                            "Resource Efficiency", "Potential Impact", "Interdisciplinary Potential",
                            "Scalability"
                        ]
                        for criterion, score in zip(criteria, list(score_vectors[idea])):
                            matchup_log += f"{criterion}: {score:.1f}\n"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max-ideas", type=int, default=None,
                        help="Maximum total ideas allowed (default: max(30, initial_ideas+10))")
    parser.add_argument("--no-debug", action="store_true", 
                        help="Disable debug logging (default: debug logging enabled)")
    ...
    print(f"Idea bounds: min={min_ideas}, max={max_ideas}")
